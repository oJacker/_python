...
def _valid_price(price):
    return (((price.max()-price.min()) / price.min()) < 0.223).all()
# 按日期分组
...
new_index = pd.date_range(start=start,end=end)
# print(new_index)

data = days.reindex(new_index) # 重新索引
...
#print( group.head())
date_col = pd.DataFrame({'group_index':group_index, 'date':new_index})
# print(date_col)
