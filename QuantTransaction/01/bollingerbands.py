...
print("deviation = ",len(deviation), "sma = ",len(sma))
upperBB = sma + deviation
lowerBB = sma - deviation

close_slice = close[N-1:]
print('close_slice= ',close_slice)
between_bands = np.where((close_slice < upperBB) & (close_slice > lowerBB))
print("close[between_bands] =", close[between_bands])
print("upperBB[between_bands]",upperBB[between_bands])
between_bands = len(np.ravel(between_bands))
print("Ratio between bands", float(between_bands)/len(close_slice))
#(2) 使用如下代码绘制布林带
t = np.arange(N -1, C)
plot(t, close_slice, lw =1.0)
plot(t, sma, lw=2.0)
