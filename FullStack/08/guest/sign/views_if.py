...
def add_event(request):
    ...
    if id == '' or name == '' or limit == '' or address == '' or start_time == '':
    ...
    if status == '':
        status = 1
    try:
        Event.objects.create(id=id, name=name, limit=limit, address=address,
...
def add_guest(request):
    id = request.POST.get('id', '')  # 关联发布会 id
    ...
    guest_limit = Guest.objects.filter(event_id=id)  # 发布会已添加的嘉宾数
    if len(guest_limit) >= event_limit:
        return JsonResponse({'status': 10024, 'message': 'event number is full'})
    event_time = Event.objects.get(id=id).start_time  # 发布会时间
