...
import time
# 三中基本的操作模式r(只可读), w（只可写） a (追加)

#流程  ：1 创建文件对象 2调用文件方法进行操作  3 关闭文件
...
#data = f.read(5)
# 昨夜寒蛩不
#print(data)  # 注意 ：汉字在这里占一个单位（in Py3）
...
#with 同时管理多个文件对象  with 库里面会自动close
with open('files','r',encoding='utf8') as f_read, open('log2','w',encoding='utf8') as f_write:
    for line in f_read:
        f_write.write(line)